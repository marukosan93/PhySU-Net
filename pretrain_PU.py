'''
SSL Pre-Training PhySUNet on specific dataset and fold, trained model can then be used for transfer learning. Same as training, only that labels and inputs are switched
Image reconstruction has masked input and mstmap as label, regression has pseudohr as label.
'''
...
def train(train_loader, model, criterion_hr, criterion_rppg, criterion_fft, optimizer, epoch):
    ...
    for i, (mstmap, masked_map, bvpmap,hr_bvp,chrom_hr,fps_,bvp,idx) in enumerate(train_loader):
        ...
        chrom_hr = chrom_hr.to(device)

        #Forward pass
        output, out_hr, feat = model(masked_map)

        loss_rppg = criterion_rppg(output,mstmap)
        loss_fft = criterion_fft(output,mstmap,fps,f_min,f_max)
...
parser.add_argument('-n', '--name', type=str, required=True) #string to identify run, e.g pretrain
